...
$filterQuery = "";
if ($filter == "watching") $filterQuery = "AND user_list.status='watching'";
if ($filter == "done") $filterQuery = "AND user_list.status='done'";

$list = mysqli_query($conn, "
    SELECT anime.*, user_list.status 
    FROM anime 
...
<head>
    ...
    <style>
        ...
        .profile-pic {
            width: 100px; height: 100px; border-radius: 50%; overflow:hidden; margin-bottom:10px;
            background: rgba(255,255,255,0.1); display:flex; align-items:center; justify-content:center;
        }
        .profile-pic img { width:100%; height:100%; object-fit:cover; }
        .no-pic-icon { font-size:40px; color:#ccc; }
        .username { font-weight:bold; margin-bottom:10px; text-align:center; }
        ...
        .header .welcome { font-size:20px; margin-top:5px; color:#ddd; }
        ...
        .anime-actions { margin-top:10px; display:flex; flex-wrap:wrap; gap:5px; }
        .anime-actions a {
            flex:1 1 45%;
            text-align:center;
            padding:6px 0;
        ...
        .no-anime { margin-top:20px; font-size:16px; text-align:center; }
    </style>
</head>
<body>
...
<div class="main-content">
    ...
    <div class="filter-buttons">
        <a href="user_mylist.php?filter=all" class="<?= $filter=='all'?'active':'' ?>">All</a>
        <a href="user_mylist.php?filter=watching" class="<?= $filter=='watching'?'active':'' ?>">Watching</a>
        <a href="user_mylist.php?filter=done" class="<?= $filter=='done'?'active':'' ?>">Done</a>
    </div>

    <?php if(mysqli_num_rows($list) === 0): ?>
        <div class="no-anime">No anime found in this category.</div>
    <?php else: ?>
            ...
            <div class="anime-card">
                ...
                <div class="anime-info">
                    ...
                    <div class="anime-actions">
                        <a href="user_anime_view.php?id=<?= $m['id'] ?>">View</a>
                        <?php if ($m['status'] == "watching"): ?>
                            <a href="user_update_status.php?id=<?= $m['id'] ?>&status=done">Mark as Done</a>
                        <?php else: ?>
                            <a href="user_update_status.php?id=<?= $m['id'] ?>&status=watching">Mark as Watching</a>
                        <?php endif; ?>
