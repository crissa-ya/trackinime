...
if ($search !== '') {
    ...
    $allAnime = mysqli_query($conn, "SELECT * FROM anime WHERE title LIKE '%$searchEscaped%' OR genre LIKE '%$searchEscaped%'");
} else {
    // Fetch by section
...
<html>
<head>
    <title>User Dashboard - TrackiNime</title>
    <style>
        body {
            margin:0;
        ...
        .sidebar {
            position:fixed; left:0; top:0; width:220px; height:100%; background:rgba(0,0,0,0.85);
        ...
        .header a.contact { position:absolute; top:20px; right:40px; padding:8px 15px; background: rgba(120,105,255,0.8); border-radius:6px; color:white; }

        .search-bar { margin-bottom:20px; display:flex; gap:10px; align-items:center; }
        .search-bar input[type="text"] { padding:8px; width:300px; border-radius:6px; border:none; }
        .search-bar button { padding:8px 12px; border-radius:6px; border:none; background: rgba(120,105,255,0.8); color:white; cursor:pointer; }
        .search-bar button:hover { background: rgba(110,95,245,1); }
        .back-search { padding:8px 12px; background: rgba(255,75,75,0.8); border-radius:6px; color:white; font-size:14px; transition:0.2s; }
        .back-search:hover { background: rgba(255,60,60,1); }

        .section { margin-top:30px; }
        .section h2 { margin-bottom:15px; }

        .anime-list { display:flex; 
            flex-wrap:wrap; 
            gap:15px; }
        .anime-card { 
...
<div class="sidebar">
    <a href="user_dashboard.php"><img src="assets/logo.png" class="logo"></a>
    <!-- Sidebar Profile -->
    ...
    <div id="profileModal" class="modal">
        <div class="modal-content">
            ...
            <form id="uploadForm" action="user_profile.php" method="POST" enctype="multipart/form-data" style="display:none;">
    ...
    // Open modal on click
    profilePic.addEventListener('click', () => { modal.style.display = 'flex'; });
...
<div class="main-content">
    ...
    <div class="search-bar">
        <form method="GET">
            <input type="text" name="search" placeholder="Search anime..." value="<?= htmlspecialchars($search) ?>">
            <button type="submit">Search</button>
            <?php if($search !== ''): ?>
                <a href="user_dashboard.php" class="back-search">← Back</a>
            <?php endif; ?>
        </form>
    </div>
    ...
    <?php if($search !== ''): ?>
        ...
        <?php if(mysqli_num_rows($allAnime) === 0): ?>
            <div class="no-anime">No anime found.</div>
        <?php else: ?>
            <div class="anime-list">
                <?php while($a = mysqli_fetch_assoc($allAnime)): ?>
                <div class="anime-card">
                    ...
                    <div class="anime-info">
                        ...
                        <div class="anime-actions">
                            <a href="user_anime_view.php?id=<?= $a['id'] ?>&from=<?= urlencode($_SERVER['REQUEST_URI']) ?>">View</a>
                            <a href="user_add.php?id=<?= $a['id'] ?>&from=<?= urlencode($_SERVER['REQUEST_URI']) ?>">Add to My List</a>
                        </div>
                    </div>
                </div>
                <?php endwhile; ?>
            ...
            <div class="anime-list">
                ...
                <div class="anime-card">
                    ...
                    <div class="anime-info">
                        ...
                        <p><strong>Rating:</strong> <?= htmlspecialchars($a['rating']) ?></p>
                        <div class="anime-actions">
                            <a href="user_anime_view.php?id=<?= $a['id'] ?>">View</a>
                            <a href="user_add.php?id=<?= $a['id'] ?>">Add to My List</a>
                        </div>
                    </div>
                </div>
                <?php endwhile; ?>
...
<?php if(isset($_GET['status'])): ?>
    ...
    <?php elseif($_GET['status'] == 'exists'): ?>
        message = "This anime is already in your list!";
    <?php endif; ?>
